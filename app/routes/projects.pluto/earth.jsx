...
import earthModel from '~/assets/cigar-client.glb';
...
export const Earth = ({
  ...
  useEffect(() => {
    ...
    const ambientLight = new AmbientLight(0x222222, 0.005);
    const dirLight = new DirectionalLight(0xffffff, 2.5);
    dirLight.position.set(1, 5, 15);
  ...
  useEffect(() => {
    ...
    const loadModel = async () => {
      ...
      sceneModel.current.position.y = position[1] - 0.5;
      sceneModel.current.position.z = position[2];

      sceneModel.current.scale.x = scale / 10;
      sceneModel.current.scale.y = scale / 10;
      sceneModel.current.scale.z = scale / 10;
    };
